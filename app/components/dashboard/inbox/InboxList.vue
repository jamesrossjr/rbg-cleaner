  ...
  <div class="overflow-y-auto divide-y divide-(--ui-border)">
    ...
    >
      <div
        ...
        :class="[
          !mail.read ? 'text-(--ui-text-highlighted)' : 'text-(--ui-text-toned)',
      ...
      >
        <div class="flex items-center justify-between" :class="[!mail.read && 'font-semibold']">
          <div class="flex items-center gap-3">
            {{ mail.from.name }}

            <UChip v-if="!mail.read" />
          </div>

          <span>{{ isToday(new Date(mail.date)) ? format(new Date(mail.date), 'HH:mm') : format(new Date(mail.date), 'dd MMM') }}</span>
        </div>
        <p class="truncate" :class="[!mail.read && 'font-semibold']">
          {{ mail.subject }}
        </p>
